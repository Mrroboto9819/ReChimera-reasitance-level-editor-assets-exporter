...
//! - `+0x5E` `u16 numVerties`
//!
//! ## RegionVertex (20 bytes)
//! - `+0x00` `i16 pos[4]` — R16G16B16A16 NORM (the .w slot is unused)
//! - `+0x08` `f16 uv0[2]`
//! - `+0x0C` `f16 uv1[2]`
//! - `+0x10` `u32 normal` — R11G11B10 NORM (we drop on import)
//!
...
const REGION_VERTEX_STRIDE: usize = 0x14;
...
pub fn read_regions_rfom(level_folder: &Path) -> Result<Vec<Zone>> {
    ...
    for i in 0..count {
        ...
        ufrags.push(UFrag {
            tuid: i as u64,
            position: [
                region_pos_x / POS_NORM_DIV * YARD_TO_M,
                region_pos_y / POS_NORM_DIV * YARD_TO_M,
                region_pos_z / POS_NORM_DIV * YARD_TO_M,
            ],
            radius: 0.0,
